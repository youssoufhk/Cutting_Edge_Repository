...
noise_size = X_size
batch_size = 16               #the number of date we will used for one network training
nb_batch = int(X_size / batch_size)
epochs = 1000              #the number of iteration
...
def generator(Z,nb_neurone=20,reuse=False):
    """ generator structure
    Args:
...
def discriminator(X,nb_neurone=20,reuse=False):
    """ generator structure
    Args:
...
with tf.device('/device:GPU:0'):
    for i in range(epochs):
        ind_X = random.sample(range(X_size),X_size)
        ...
        print ("Iterations:" ,i,"Discriminator loss: ",dloss,"t Generator loss:" , gloss)
...
#Check the cov matrix (problem need to be solved)
Cov_pred = np.corrcoef(pred)
#
Cov_X = np.corrcoef(X_batch)
